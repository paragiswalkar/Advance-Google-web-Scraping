...
<?php
	...
	$get_proxys = isset($_GET['proxy1'])?$_GET['proxy1']:NULL;
			...
			echo "Unable to open file";	
                ...
                if ($info['timed_out'])
                {
					...
					$data = extractBody($res);
                                        ...
                                        $test_website_url = '';
					$domain_name = isset($_POST['domain_a'])?explode(',', $_POST['domain_a']):NULL;
                                        
                                        for($i=0;$i<count($domain_name);$i++){
                                            $test_website_url[] = $domain_name[$i];
                                            echo "$NL$B Search Engine Scraper for $test_website_url[$i] initated $B_ $NL$NL"."<br>";
                                        }
                                        ...
                                        for($j=0;$j<count($test_website_url);$j++){
                                                    ...
                                                    */
                                                    for ($page = 0; $page < $test_max_pages; $page++)
                                                    {
                                                            ...
                                                            if (!$serp_data)
                                                            {
                                                                    ...
                                                                    if (!$load_all_ranks)
                                                                    {
                                                                            for ($n = 0; $n < $result_count; $n++)
                                                                                    if (strstr($results[$n]['url'], $test_website_url[$j]))
                                                                                    {
                                                                                            verbose("Located $test_website_url[$j] within search results.$NL");
                                                                                            $break=1;
                                                                                            //break;
                                                                                    }
                                                                    }
                                                            ...
                                                            for ($ref = 0; $ref < $result_count; $ref++)
                                                            {
                                                                    ...
                                                                    if (strstr($rank_data[$keyword][$rank]['url'], $test_website_url[$j]))
                                                                    {
                                                                            $info = array();
                                                                            $info['rank'] = $rank;
                                            ...
                                            if ($show_all_ranks)
                                            {
                                                foreach ($rank_data as $keyword => $ranks)
                                                {
                                                    ...
                                                    {
                                                        $pos++;
                                                        if (strstr($rank['url'], $test_website_url[$j]))
                                                        {
                                                                            echo "<tr class=$class $style>
                                                                                                    ...
                                                                                                    <td>$rank[desc]</td>
                                                                                                    <td><a href='".$rank['url']."' id='".'link_'.$pos."' target='_blank'>$rank[url]</a></td>
                                                                                                    <td>$rank[host]</td>
                                                                                                    <td><iframe id='".$pos."' src=".$rank[url]." onLoad='iframeDidLoad();' style='width:500px;height:250px;'></iframe></td>
                                                                                            </tr>";
                                            ...
                                            //                    echo $rank['desc']."\n";
                                                                            ...
                                                                            echo '<script>
                                                                                                    ...
                                                                                                    function iframeDidLoad(){
                                                                                                            document.getElementById("'.$pos.'").src = sites[Math.floor(Math.random() * sites.length)];
                                                                                                    }
                                                                                            </script>';
                                                        } else
                                                        {
                                            ...
                                            foreach ($keywords as $keyword)
                                            {
                                                ...
                                                } else
                                                {
                                                    ...
                                                    echo "$NL$NL$B" . "Ranking information for keyword \"$keyword\" and website \"$test_website_url[$j]\" [$test_country / $test_language] $B_$NL"."<br>";
